...
function getUpstashRedisClient(): Redis {
  ...
  if (!client) {
    const upstashUrl = process.env.UPSTASH_URL || process.env.KV_REST_API_URL || process.env.UPSTASH_REDIS_REST_URL;
    const upstashToken = process.env.UPSTASH_TOKEN || process.env.KV_REST_API_TOKEN || process.env.UPSTASH_REDIS_REST_TOKEN;

    if (!upstashUrl || !upstashToken) {
      throw new Error(
        'UPSTASH_URL and UPSTASH_TOKEN (or KV_REST_API/UPSTASH_REDIS_REST) env variables must be set'
      );
    }
